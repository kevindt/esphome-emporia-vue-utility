...
class EmporiaVueUtility : public Component,  public UARTDevice {
    public:
        ...
        // Uses data retrieved via the second UART connected to the MGM111
        // Creates four sensors that publish data via MQTT
        ...
        // Reads and logs everything from serial until it runs out of data or encounters a 0x0d byte (ascii CR)
        // Primarily for debug/error logging
        ...
        // parse out the watt-hours value (meter running total) from the meter reading struct
        // and calculate a running average over the prior N values
        // It can increase (consumption) or decrease (production) for a NEM meter
        float parse_meter_watt_hours(struct MeterReading *mr) {
            ...
            kWh_net->publish_state(watt_hours / 1000.0);  // publish meter running total to MQTT

            return(watt_hours);
        }   

        // Parse out the instantaneous watts value from the meter reading struct - it can be positive (consumption)
        // or negative (production) for a NEM meter
        float parse_meter_watts(struct MeterReading *mr) {
            ...
            if ((watts >= WATTS_MAX) || (watts < WATTS_MIN)) {
                ...
                W->publish_state(watts);  // publish instantaneous watts value to MQTT
            }
            return(watts);
        } 
...
};  // end of Class EmporiaVueUtility 
